...
### `--ccs`
Set defaults to `-A 4 -B 1 -D 3 -I 3 -X 3 --per-read`
...
but it happens that some ZMWs do not pass the `--signal-increase` threshold.
If there are too many *same* barcode pairs, try to relax `--signal-increase`,
leading to potentially false positive calls.
